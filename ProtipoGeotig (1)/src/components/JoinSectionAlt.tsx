  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (timeSinceLastSubmit < 10000) {
        toast.error('Por favor espera unos segundos antes de enviar otra solicitud.');
        return;
      }
      ...
      if (submitTimeWindow.current.length > 3) {
        toast.error('Has alcanzado el límite de envíos. Por favor intenta más tarde.');
        return;
      }
      ...
      if (sanitizedData.nombre.length < 3 || sanitizedData.nombre.length > 100) {
        toast.error('El nombre debe tener entre 3 y 100 caracteres.');
        return;
      }

      if (sanitizedData.motivacion.length < 20 || sanitizedData.motivacion.length > 500) {
        toast.error('La motivación debe tener entre 20 y 500 caracteres.');
        return;
      }

      // 6. Verificar que los datos sanitizados no estén vacíos
      if (!sanitizedData.nombre || !sanitizedData.email || !sanitizedData.programa || !sanitizedData.motivacion) {
    ...
    <section id="unete" className="py-16 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-4xl mx-auto mb-12">
          ...
          >
            <span className="text-emerald-600 uppercase tracking-widest text-lg font-semibold">¡Es tu momento!</span>
          </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="bg-gradient-to-br from-blue-50 to-emerald-50 rounded-3xl p-8 lg:p-10 border-2 border-emerald-100">
            ...
            <div className="mt-8 p-6 bg-white rounded-xl border-2 border-emerald-200">
              <p className="text-sm text-gray-600 italic">
                "El semillero GEOTIG me permitió desarrollar mis habilidades en SIG y publicar mi primer artículo científico. Una experiencia transformadora."
              </p>
              <div className="mt-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500">Estudiante Ingeniería Civil</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-950 to-emerald-900 rounded-3xl p-8 lg:p-10 text-white">
              ...
              <div className="mb-6 p-4 bg-emerald-500/20 border border-emerald-400/50 rounded-lg flex items-start gap-3">
                ...
                <div>
                  <p className="text-emerald-100 text-sm">
                    ¡Solicitud enviada exitosamente! Revisaremos tu información y nos pondremos en contacto pronto.
                  </p>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div>
                ...
                {errors.nombre && (
                  <div className="flex items-start gap-2 mt-2">
                    ...
                    <p className="text-red-300 text-sm">{errors.nombre.message}</p>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                {errors.programa && (
                  <div className="flex items-start gap-2 mt-2">
                    <AlertCircle className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" />
                    <p className="text-red-300 text-sm">{errors.programa.message}</p>
                  </div>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm text-gray-200 mb-2">
                  ¿Por qué quieres unirte? <span className="text-red-400">*</span>
                </label>
                <Textarea 
                  placeholder="Cuéntanos sobre tus intereses en GIS, investigación territorial, proyectos que te gustaría desarrollar..."
                  rows={4}
                  className={`bg-white/10 border-white/20 text-white placeholder:text-gray-400 ${
                    errors.motivacion ? 'border-red-400 focus:border-red-400' : ''
                  }`}
                  {...register('motivacion', {
                    required: 'Por favor cuéntanos tu motivación para unirte',
                    minLength: {
                      value: 20,
                      message: 'Por favor escribe al menos 20 caracteres describiendo tu motivación'
                    },
                    maxLength: {
                      value: 500,
                ...
                {errors.motivacion && (
                  <div className="flex items-start gap-2 mt-2">
                    <AlertCircle className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" />
                    <p className="text-red-300 text-sm">{errors.motivacion.message}</p>
                  </div>
                )}
              </div>
            ...
            <div className="mt-6 pt-6 border-t border-white/20 text-center">
              <p className="text-sm text-gray-300 mb-2">También puedes contactarnos por:</p>
              <a href="mailto:geotig@unicauca.edu.co" className="inline-flex items-center gap-2 text-emerald-300 hover:text-emerald-200 transition-colors">
